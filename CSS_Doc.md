...
## SVG
HTMLに直接パスなどが記述されたSVGはCSSから属性を変えることでアニメーションを行うことができる
### 【主な属性】
* fill→塗りつぶしの色
* fill-opacity→透過度
* stroke→線の色
...
* cx,cy→円形の座標
### アニメーション関係
* animationやtransitionなどと組み合わせてアニメーションを作ることができる
* パスには書き方、書き順などの情報が含まれており、それをアニメーションとして表示することが出来る
* SVGにCSSを埋め込むことでアニメーションSVGファイルが出来る
### imgタグとobjectタグ
